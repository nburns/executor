...
char ROMlib_rcsid_launch[] =
	    "$Id: launch.c 88 2005-05-25 03:59:37Z ctm $";
#endif
...
#define SetTitle(x) do { ROMlib_WindowName = x; } while (0)
...
PUBLIC cfir_t *
ROMlib_find_cfrg (Handle cfrg, OSType arch, uint8 type, Str255 name)
{
...
PUBLIC launch_failure_t ROMlib_launch_failure = launch_no_failure;
PUBLIC INTEGER ROMlib_exevrefnum;
...
#define LEMMINGSHACK
	...
	ROMlib_find_cfrg (cfrg0, T('p','w','p','c'), kApplicationCFrag,
			  (StringPtr) ""))
      code0 = NULL;
    else if (!code0)
      {
...
NewLaunch (StringPtr fName_arg, INTEGER vRefNum_arg, LaunchParamBlockRec *lpbp)
{
	...
	filenames = alloca (n_filename_bytes);
