...
PUBLIC void
replace_physgestalt_selector (OSType selector, uint32 new_value)
{
  REPLACE_SELECTOR_IN_TABLE (selector, new_value, phystable);
}

PUBLIC void
gestalt_set_memory_size (uint32 size)
{
...
PUBLIC void
gestalt_set_system_version (uint32 version)
{
...
PUBLIC void
ROMlib_clear_gestalt_list (void)
{
  free (gestalt_listp);
  gestalt_listp = NULL;
  listp_size = 0;
...
PUBLIC void
ROMlib_add_to_gestalt_list (OSType selector, OSErr retval, uint32 new_value)
{
  typeof (gestalt_listp) new_listp;

  new_listp = realloc (gestalt_listp, listp_size + sizeof *gestalt_listp);
  if (!new_listp)
   ...
   ProcPtr *, oldSelFuncp)
{
  ...
  if (gp)
	...
	{
	  *oldSelFuncp = P_GestaltTablesOnly;
	  retval = new_link(selector, selFunc);
	}
    }
  return retval;
