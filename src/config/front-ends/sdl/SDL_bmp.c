...
#ifdef SAVE_RCSID
static char rcsid =
 "@(#) $Id: SDL_bmp.c 74 2004-12-30 03:38:55Z ctm $";
#endif
/* 
...
{
  obj->error = (obj->error ||
		fseek (obj->u.bmp.fp, obj->u.bmp.bfOffBits, SEEK_SET) != 0);
}
...
cfdib_write (dib_write_obj_t *obj, const void *p, int n_bytes)
{
  ...
  if (!obj->error)
      ...
      if (!obj->error)
	{
	  ...
	  new_n_bytes_used = ((char *) obj->u.cfdib.p -
			      (char *) obj->u.cfdib.bytes);
...
{
  obj->error = (obj->error ||
		fseek (obj->u.bmp.fp, offset, SEEK_SET) != 0);
}
...
int
SDL_SaveBMP_ctmhackage (SDL_Surface *surfp, const char *file)
{
  int retval;
...
int
SDL_SaveCF_DIB (SDL_Surface *surfp, char **dib_bytesp, size_t *dib_lenp)
{
  int retval;
